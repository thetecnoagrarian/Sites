...
router.post('/dashboard/posts/create', isAdmin, (req, res, next) => {
    upload.array('image', 50)(req, res, function (err) {
      if (err instanceof multer.MulterError) {
        req.flash('error', 'One or more files are too large. Max size is 50MB per file.');
...
router.post('/dashboard/posts/:id/update', isAdmin, upload.array('image', 50), async (req, res) => {
    try {
        const Post = require('../models/post');
        const { processImage } = require('../utils/imageProcessor');
        const post = Post.findById(req.params.id);
