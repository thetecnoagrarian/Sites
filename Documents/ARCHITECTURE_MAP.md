...
- `init.js` creates parent directories, opens SQLite through `better-sqlite3`, enables foreign-key enforcement for shared application connections, and initializes an empty database transactionally at the current schema. Fresh initialization records the canonical baseline and every current migration. It does not apply pending migrations to an existing schema.
- `schema.sql` defines the current shared schema: login users; posts; categories; post-category relationships; sessions; public people; one-based ordered post-author relationships; publication/review fields; authorship review guards; and legacy update triggers.
...
- `migrations/0001_public_author_publication_model.sql` is the first additive upgrade. It creates public Person identities and one-based ordered post authorship, adds a nullable per-post Person publisher, separates exact and date-only publication facts, reserves nullable `modified_at`, and records separate authorship/publication review state. A neutral Person may be an author, a publisher, or both; authorship is expressed by the post-author relationship. It does not derive facts from legacy fields.
- `schema-recognition.js` compares shared columns, check constraints, indexes, foreign keys, and normalized trigger behavior using SQLite metadata. It recognizes the canonical baseline, the verified TTA variant with `categories.description`, and the verified FFG variant with `users.role` added last. Formatting and comments do not define a separate schema state; unknown material drift still fails.
- `migrations.js` runs explicitly requested, ordered SQL migrations with a per-database `schema_migrations` ledger. A migration may contain multiple statements, while the runner owns the transaction and rejects transaction-control SQL. The companion `schema_baseline` record preserves which supported starting variant was observed; database guards prevent replacing it after creation. The `0000_existing_schema` ledger entry is a lineage checkpoint for legacy variants, not a claim that their tables were literally created from `0000`. `migrate-cli.js` exposes read-only `inspect`/`plan` and explicit `apply` commands.

Application startup initializes an empty database at the current schema; it inspects recognized existing variants and fails on unsupported drift, but does not apply pending migrations. Future schema changes must be added as ordered migration files and applied explicitly to each existing database before code that requires them is started. TTA's historical category description remains present. The active models use named columns/properties, so FFG's `users.role` ordinal position remains unchanged and does not affect current application access.
