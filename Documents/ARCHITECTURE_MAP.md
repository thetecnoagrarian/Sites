...
- `schema-recognition.js` compares shared columns, constraints, indexes, foreign keys, and normalized trigger behavior using SQLite metadata. It recognizes the canonical baseline, the verified TTA variant with `categories.description`, and the verified FFG variant with `users.role` added last. Formatting and comments do not define a separate schema state; unknown material drift still fails.
- `migrations.js` runs explicitly requested, ordered SQL migrations with a per-database `schema_migrations` ledger. The companion `schema_baseline` record preserves which supported starting variant was observed; database guards prevent replacing it after creation. The `0000_existing_schema` ledger entry is a lineage checkpoint for legacy variants, not a claim that their tables were literally created from `0000`. `migrate-cli.js` exposes read-only `inspect`/`plan` and explicit `apply` commands.

Application startup still initializes the current fresh schema; it inspects recognized existing variants and fails on unsupported drift, but does not run the migration command. Future schema changes should be added as ordered migration files and applied explicitly to all supported variants before code that requires them is started. TTA's historical category description remains present unless a separately scoped migration deliberately addresses it. The active models use named columns/properties, so FFG's `users.role` ordinal position does not affect current application access.
