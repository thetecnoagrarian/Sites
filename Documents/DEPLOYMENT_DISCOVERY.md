...
| `Documents/MASTER_PROJECT_DOCUMENTATION.md` | Broad historical project/deployment notes, older deploy examples, SSH/key/auth notes, backup/cleanup notes | historical / needs review | high | Contains useful clues that deployment was SSH plus Docker Compose based and that `[DEPLOY_PATH]` was used historically. Newer focused docs supersede it for safety. | Contains live-looking access/account/key/path/command details; use placeholders only. |
